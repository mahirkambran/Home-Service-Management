...
class home(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        all_services = list(services)  # No need to query twice

        sample_size = min(len(all_services), 3)
        selected_services = random.sample(all_services, sample_size)
...
class AssignWorker(LoginRequiredMixin, View):
    ...
    def get(self, request, id):
        ...
        # Get available workers for this service (case-insensitive)
        workers_records = workers.objects.filter(designation__iexact=service_name, avalability_status=True)

        # Fallback: show all workers for this service if none available
        if not workers_records.exists():
            workers_records = workers.objects.filter(designation__iexact=service_name)

        # Debug: print for verification
        print("Requested service:", service_name)
        print("Workers found:", list(workers_records.values('id','admin__first_name','admin__last_name','designation','avalability_status')))

        context = {
            'req': req,
            'workers': workers_records,
        }
    ...
    def post(self, request, id):
        ...
        if not worker_id:
            return HttpResponse("Please select a worker.", status=400)

        try:
            assigned_worker = workers.objects.get(id=worker_id)
        except workers.DoesNotExist:
            return HttpResponse("Selected worker does not exist.", status=400)
        ...
        assigned_worker.avalability_status = False
